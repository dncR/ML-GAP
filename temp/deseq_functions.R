...
preProcessCounts <- function(object, normalize = TRUE, transform = TRUE, 
                             transformationMethod = c("vst", "rlog", "logCPM"), ...){
  ...
  if (normalize){
    ...
    if (!inherits(success, "try-error")){
      normalizedCounts <- counts(object, normalize = TRUE)  
    } else {
  ...
  if (transform){
    if (transformationMethod == "vst"){
      success <- try({
        object <- varianceStabilizingTransformation(object)
      })
      
      if (!inherits(success, "try-error")){
        vstCounts <- assay(object)  
      } else {
        vstCounts <- NULL
      }
    }
